...
module.exports = grunt => {
  ...
  grunt.initConfig({
    ...
    jshint: {
      ...
      options: {
        jshintrc: ".jshintrc",
        ignores: ["public/js/libs/*.js", "public/spec-e2e/*"]
      }
    },
          ...
          append: [
            {
              selector: "head",
              html:
                '<link href="<%= distUrl.prod %>/css/<%= pkg.name %>-<%= pkg.version %>.css" rel="stylesheet">'
            }
          ]
        },
        // overwrite file
    ...
    cssmin: {
      build: {
        ...
        files: {
          "<%= distUrl.prod %>/css/<%= pkg.name %>-<%= pkg.version %>.css": [
            "temp/style.css"
          ]
        }
      }
    ...
    uglify: {
      build: {
        ...
        files: {
          "<%= distUrl.prod %>/js/<%= pkg.name %>-<%= pkg.version %>.js": [
            "temp/script.js"
          ]
        }
      }
  ...
  grunt.registerTask("default", ["jshint", "mocha_istanbul", "karma"]);
  ...
  grunt.registerTask("bdd", ["protractor:bdd"]);
  ...
  grunt.registerTask("deploy", "Deploy bulk commands", version => {
    ...
    grunt.task.run(["exec:npm", "exec:bower", "exec:build", "exec:restart"]);
  });
};
